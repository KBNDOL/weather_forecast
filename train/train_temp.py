...
scaler_target_temp = MinMaxScaler()
data_scaled_target_temp = scaler_target_temp.fit_transform(data[['future_temp']])



X = np.reshape(data_scaled_features, (data_scaled_features.shape[0], 1, data_scaled_features.shape[1]))
y_temp = data_scaled_target_temp
...
output_temp = Dense(1, name='temp_output')(lstm_layer)


model = Model(inputs=input_layer, outputs=[output_temp])
model.compile(optimizer='adam', loss={'temp_output': 'mean_squared_error',})

history = model.fit(X_train, {'temp_output': y_train_temp}, epochs=64, batch_size=32, validation_split=0.1, verbose=2)

model.save('D:/pythonProjectTotal/weather_forecast/model/forecast_temp.keras')
joblib.dump(scaler_features, 'D:/pythonProjectTotal/weather_forecast/scaler/scaler_features.pkl')
joblib.dump(scaler_target_temp, 'D:/pythonProjectTotal/weather_forecast/scaler/scaler_target_temp.pkl')
