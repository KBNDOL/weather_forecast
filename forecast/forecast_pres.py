...
def predict_pres_from_excel(excel_path):
    # 加载保存的模型和归一化对象
    pres_model = load_model('model/forecast_pres.keras')
    scaler_features = joblib.load('scaler/scaler_features.pkl')
    scaler_target_temp = joblib.load('scaler/scaler_target_pres.pkl')
